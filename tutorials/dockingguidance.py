...
try:
    ...
    while True:
        ...
        if current is None:         # 如果飞行器还没对接就显示等待对接
            stdscr.addstr(2, 0, 'Awaiting control from docking port...')

        elif target is None:        # 如果目标对接口还没对接，就显示等待对接
            ...
            displacement = np.array(current_position)       # 把 tuple 类型的 position 改成 array 类型的
            distance = la.norm(displacement)                # 定义两个对接口之间的距离
            speed = la.norm(np.array(velocity))             # 求出相对速率
